...
ds_with_embeddings['train'].save_faiss_index('embeddings', 'faiss_index/quanta_psgs.faiss')

map_embeddings = {}
count = 1
for i in tqdm(range(0, len(ds_with_embeddings['train']))):
...
ds_with_embeddings_avg = ds.map(lambda example: {'embeddings': map_embeddings[example['title']]})
ds_with_embeddings_avg['train'].add_faiss_index(column='embeddings')
ds_with_embeddings_avg['train'].save_faiss_index('embeddings', 'faiss_index/quanta_psgs_avg.faiss')
